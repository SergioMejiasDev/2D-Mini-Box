...
public class FrogMovement : MonoBehaviour
{
    ...
    void CheckInputs()
    {
        ...
        if (Input.GetKeyDown(KeyCode.D) && !RayMovement(Vector2.right))
        {
            jumpSound.Play();
            ResetValues();
        ...
        if (Input.GetKeyDown(KeyCode.S) && !RayMovement(-Vector2.up))
        {
            jumpSound.Play();
            ResetValues();
            destination = (Vector2)transform.position - 0.5f * Vector2.up;
        ...
        if (Input.GetKeyDown(KeyCode.A) && !RayMovement(-Vector2.right))
        {
            jumpSound.Play();
            ResetValues();
            destination = (Vector2)transform.position - 0.45f * Vector2.right;
